...
\begin{tikzpicture}[thick]%,show background grid]
	...
	% rotation axis
		\draw[ultra thick, ->] (0,-3) ++ (-50:.75) arc (-50:300:.75 and .25);
		\draw[ultra thick] (0,-4) node[below] {Rotation axis} -- ++(0,3);
		\draw[ultra thick,dashed] (0,-1) -- ++(0,2);
	% position 1
	...
	% position 2
		...
		\draw (2.5,-3) node[right] {Second Position};
	% top from position 1 on top
		\draw (0,1) circle (1.5 and .5);
	% rotation axis on top
		\draw[ultra thick,->] (0,1) -- ++(0,2.5);									
	% sample movement
		\draw[->] (0,1) -- (3,1) node [text width=1.75cm,above] {Sample movement relative to beam};	
\end{tikzpicture}
%\end{preview}
%\end{document}